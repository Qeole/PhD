...
\newcommand{\sectionheader}[1]{%
  \begin{tikzpicture}
    \node[draw,color=chapterLACL!20,fill=chapterLACL!20,inner sep=8pt] (sectiontitle) {\begin{minipage}{\sectionnode}\color{white}\color{black}\thesection \quad #1\end{minipage}};
    \fill[fill=chapterLACL] (sectiontitle.north west) -- ++(0,8pt) -- ++(3cm,0) -- +(0.3cm,-8pt) -- cycle;
  \end{tikzpicture}%
}
\titleformat{\section}
...
\newcommand{\newthesubsection}{%
  \begin{tikzpicture}[baseline=(noexo.base)]
    ...
    \filldraw[color=chapterLACL, rounded corners=1pt] (noexo.south west) -- (noexo.north west) -- ($ (noexo.north east) + (6pt,0)$) -- ($ (noexo.south east) + (3pt,0)$) -- cycle;
    \node[right,inner sep=3pt] {\color{white}\thesubsection};
  \end{tikzpicture}%
