...
Le concept initial de l'algorithme est simple: à chaque renouvellement du processus, l'énergie résiduelle des capteurs est évaluée, et ceux possédant le plus de réserves deviennent \cns.
...
La \figref{se:fig:cover} illustre ce problème.
\begin{figure}[ht]
    \centering
...
\begin{enumerate}
    ...
    \item Le \CH sélectionne les $n$ \cns parmi les nœuds ayant le plus d'énergie résiduelle, de telle façon que ces $n$ nœuds couvrent tous les autres nœuds en terme de portée;
...
Il est intéressant de noter que certains algorithmes de clusterisation\index{clusterisation!protocole de clusterisation} (\heed~\cite{YF04} par exemple) utilisent d'autres mécanismes de sélection basés sur l'énergie résiduelle des nœuds ---~pour choisir des \chs, mais qui pourraient être transposer à la sélection des \cns.
