...
Les capteurs échangent par communications hertziennes, en utilisant des protocoles tels ceux définis dans la pile \idx{IEEE~802.11}.
Le routage des paquets dans le réseau peut faire appel à l'un des nombreux protocoles développés à cet effet (par exemple: \idx{AODV}, \idx{OLSR}), qu'il repose sur un algorithme centralisé (dirigé par une seule entité) ou distribué (exécuté par chaque entité du réseau).
...
L'une des possibilités permettant de réduire la consommation en énergie est l'utilisation d'un réseau \index{clusterisation hiérarchique} hiérarchiquement clusterisé.
...
Parmi les plus fréquemment utilisés, on trouve des algorithmes comme \idx{\leach}~\cite{HCB00} ou \idx{HEED}~\cite{YF04}.
D'autres sont moins connus, comme \idx{FFUCA}~\cite{FL11}, qui repose sur l'exploitation de propriétés ultramétriques dans le réseau.

        \subsubsection{Clusterisation hiérarchique\index{clusterisation hiérarchique}}
...
Chaque \textit{$k$-CH} reçoit des données (provenant soit de nœuds normaux si $k$ est le dernier degré de la hiérarchie constituée, soit de \textit{$k+1$-CH}), les agrège et les transmet au \textit{$k-1$-CH} auquel il est rattaché.
        ...
        \subsubsection{Exemple: fonctionnement de l'algorithme \idx{\leach}}%
        \label{st:subsubsec:leach}
        L'un des algorithmes de clusterisation les plus simples et les plus couramment employés dans les réseaux de capteurs sans fil est l'algorithme \idx{\leach} (\textit{Low Energy Adaptive Clustering Hierarchy with deterministic \ch selection}).
...
\begin{enumerate}
    ...
    \item Les \chs auto-désignés informent leurs nœuds voisins de leur changement de statut à l'aide de messages en \textit{broadcast}.
        ...
        Pour limiter les collisions, il est fait usage de la méthode \textit{Carrier Sense Multiple Access} (\idx{CSMA}) au niveau de la couche MAC.\nomenclature{CSMA}{\textit{Carrier Sense Multiple Access}}\nomenclature{MAC}{\textit{Media Access Control}}
    ...
    \item Au vu des réponses reçues, chaque \ch calcule un «~ordre de transmission~» pour les nœuds qui l'ont rejoint.
        ...
        Dans chaque cluster, les nœuds s'adresseront donc à leur \ch à tour de rôle, selon l'ordre déterminé par le \CH, ce qui revient à utiliser la méthode appelée \textit{Time Division Multiple Access} (\idx{TDMA}).\nomenclature{TDMA}{\textit{Time Division Multiple Access}}
    \item La phase de collecte des données peut débuter.
        ...
        Les collisions entre les transmissions des nœuds de différents clusters sont évitées grâce à la méthode appelée \textit{Code Division Multiple Access} (\idx{CDMA}).\nomenclature{CDMA}{\textit{Code Division Multiple Access}}
...
Et ainsi de suite par récursivité, jusqu'à obtenir le degré de hiérarchie\index{clusterisation hiérarchique} désiré dans le réseau.
Nous appellerons $k$-\leach cet algorithme appliqué de façon à créer $k$ degrés hiérarchiques.

Seconde remarque: il est possible d'étendre \idx{\leach} pour prendre en compte l'énergie restante dont dispose chaque nœud lors de l'élection des \CH.
...
Troisième point: l'un des aspects importants de \idx{\leach} est que lors de la première étape, chaque nœud choisi d'être, ou non, un \ch pour la ronde en cours.
...
Par ailleurs, la \idx{répartition géographique} (au regard de la puissance de transmission nécessaire) idéale des \chs n'est en rien assuré.
...
Si toutefois un nœud ne parvient à capter les messages d'aucun \ch, il se déclare généralement lui-même \CH.
        ...
        \subsection{Sécurité, \idx{sureté} et \idx{résilience}}
...
Les problématiques de sécurité dans les réseaux de capteurs sont développées plus en détail dans le \chapref{ea}.
