...
\begin{proof}
Supposons que la contradiction soit vraie, \cad qu'il existe $q \in V'$ tel que $q$ n'a aucune arête sortante.
...
\todo{chech $p_e$and $p_v$}
Dans ce premier exemple simple, nous étudions les objectifs, pour le \jo, de la forme $p_e \geq c_e \wedge p_v \geq c_v$, avec $c_e,c_v \in \N$.
...
Tout d'abord, nous pouvons clairement établie que depuis un état à partir duquel au moins un des objectifs ne peut être rempli, le \jo ne peut pas gagner.
De plus, si le \ji peut forcer le \jo à atteindre l'un de ces états, alors l'objectif du \jo ne sera pas rempli.
De là nous pouvons utiliser la notion classique d'\emph{attractors} dont nous avons rappelé la définition.
...
\todo{in French, math below}
\[\textit{crédit\_initial} = 1+c+\min_{\substack{\rho \textrm{ prefix of } \mathcal{C}\\  \mathcal{C}\textrm{ cycle in } \G_v}} w_e(\rho)\]
...
Prenons, pour illustrer, l'exemple du jeu à un joueur présenté en \figref{tj:fig:infmemoryneeded}.
...
La machine doit être complète au niveau des entrées possibles, aussi faut-il autoriser $q_1$ à survenir depuis n'importe quel état mémoire (dans notre cas elle retourne à l'état mémoire initial, tandis que le jeu retourne en $q_0$).\todo{re-read this}
...
Elle serait découpée en phases qui se dérouleraient de la manière suivante: à la $k$e \todo{kième} phase, il faudrait boucler $42k$~fois sur $q_0$, puis se rendre en $q_1$; il faudrait alors boucler $k$~fois avant de retourner sur $q_0$.
...
De plus, à chaque phase, le niveau d'énergie retombe à sa valeur initiale, puis ne prend que des valeurs positives.
...
Résoudre le jeu en supposant que le \jo a une mémoire finie limitée à un nombre $k$ d'unités fourni en entrée revient à deviner sa stratégie sous forme de machine de Mealy, ce qui constitue un object exponentiel si $k$ est fournie sous forme binaire.
...
Dans les deux cas cela revient à trouver un chemin en « lasso » dans le graphe, cje qui peut être deviné.

En ce qui concerne la complexité, la procédure que nous venons de décrire est dans $\NEXPSPACE$ (qui est équivalent à $\EXPSPACE$~\cite[Chap.~20]{papadimitriou94}), \todo{although the bound is not tight}.
