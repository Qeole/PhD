...
\vfill
\lettrineh{A}{près avoir proposé} un processus de sélection aléatoire\index{selection@sélection!sélection aléatoire}, une première amélioration a été proposée pour mieux répartir la consommation en énergie dans le réseau.
...
Viennent ensuite les itérations elle-même qui marquent le \idx{renouvellement} de l'attribution du rôle de \cn, tant que le réseau est en fonctionnement.
...
Nous proposons à nouveau pour ces calculs d'utiliser le modèle de diffusion de \textsc{Rakhmatov} et \textsc{Vrudhula}, détaillé au \chapref{se}, \ssref{se:rakvru-formula}.
    ...
    \subsection{Sélection\index{selection@sélection} des \cns parmi l'ensemble $\mathit{LNÉ}$}
