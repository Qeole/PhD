...
\begin{enumerate}
    ...
    \item pour chaque nœud~$i$, le \ch réalise l'analyse suivante:
        \begin{itemize}
            \item en se basant sur un modèle mathématique adapté, le \CH calcule la consommation énergétique «effective» $ECe$ relative au débit moyen\,\todo{ou maximum? Mais attaques possibles. Ou maximum pondéré? à voir…} $\{Obs_k[j][i]\}|_{j\in \mathcal{CN}}$ observé par les \cns~$j$ voisins de~$i$ au cours de l'itération,
