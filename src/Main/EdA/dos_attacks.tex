...
Le but final de telles attaques peut être d'envoyer plus de données à la \sdb, dans le but par exemple de fausser une moyenne, ou bien de transmettre les données écoutées (attaque sur la \idx{confidentialité}) dans le voisinage à une autre machine corrompue du réseau, en vue de l'exfiltration de ces données utiles capturées.
On parle aussi parfois en anglais d'\textit{unfairness} («injustice») lorsque les spécifications des protocoles mis en place sont délibérément ignorées par un nœud corrompu.
        ...
        \paragraph{Falsification\index{falsification} d'accusés de réception (\textit{ACK spoofing})}
...
Le \idx{routage} est alors très simple.
...
Un protocole de \idx{routage} efficace doit minimiser les pertes de paquets ainsi que les couts de retransmission et éviter la création de boucles\index{routage!boucle} dans le réseau.
...
Toutes les routes\index{routage!route} qui passent par ce nœud compromis mènent donc à la suppression pure et simples des paquets en cours de transit~\cite{Kar05}.
...
On parlera plutôt d'attaques de type «trou gris»\index{trou!trou gris} (\textit{grey holes} en anglais) lorsque la retransmission est partielle mais constante dans le temps, qu'elle soit basée sur un mécanisme aléatoire (paquets supprimés au hasard plutôt que d'être retransmis) ou selon des règles (selon le contenu utile, ou selon l'expéditeur ou le destinataire par exemple)~\cite{ZTLMK13}.
        ...
        \paragraph{Falsification\index{falsification} des informations de \idx{routage} (accaparement\index{comportement cupide!accaparement} du canal de transmission, création de boucles\index{routage!boucle}, \etc)}
Une attaque peut être menée dès le déploiement du réseau en transmettant de fausses informations lors de la mise en place des règles de \idx{routage}~\cite{AD14}.
Ces informations vont alors chercher à nuire au \idx{routage} des paquets, en créant des boucles\index{routage!boucle} cycliques dans la structure de \idx{routage}, en partitionnant le réseau, en attirant à tort le trafic vers un nœud corrompu (par exemple pour exploiter les données et porter une attaque sur la \idx{confidentialité}) ou vers un nœud légitime (pour surcharger ses capacités de traitement)\dots
...
Une attaque \textit{sinkhole}\index{trou!puits} en anglais est la combinaison d'une attaque de type «trou noir»\index{trou!trou noir} avec la diffusion de fausses informations de \idx{routage}, en vue d'attirer le maximum de paquets possible vers le nœud attaquant~\cite{CT04}.
Concrètement, un nœud corrompu peut se déclarer voisin direct de la \sdb et annoncer une route\index{routage!route} de cout nul vers cette dernière.
...
Au final une part importante des routes\index{routage!route} créées, et en conséquence du trafic routé, va passer par ce nœud compromis, ce qui peut mener à des congestions\index{congestion} dans le réseau.
...
Cette attaque consiste à capturer du trafic en un point donné du réseau pour le réinjecter\index{injection} en un autre point.
...
Un attaquant, s'il dispose d'une machine plus puissante que la moyenne des capteurs, peut forger et envoyer des paquets «hello-reply» avec une portée supérieure à celle des capteurs, pour leur faire enregistrer des voisins qui sont en réalité bien au-delà de leur portée d'émission, et désorganiser complètement le \idx{routage} de paquets dans le réseau~\cite{AD14}.

        \paragraph{Altération\index{altération} des données}
Lorsqu'aucune vérification de l'\integrite des données n'est réalisée, un nœud relai corrompu peut, s'il a accès au contenu (en clair ou parce qu'il possède la clé pour déchiffrer), modifier ou retrancher des données du paquet à retransmettre~\cite{SZFDXC14}.
...
\end{itemize}
Cette mascarade permet au nœud compromis de faire échouer d'éventuels schémas de \resilience, par exemple lorsque le protocole de \idx{routage} prévoit des routes\index{routage!route} «de secours» si les routes\index{routage!route} principales venaient à être coupées.
Des schémas de partition, de réplication\index{replication@réplication}, de distribution du stockage de l'information se retrouvent également affectés si des nœuds qui sont considérés comme distincts par les agents légitimes sont en fait simulés par un unique capteur compromis.
...
Cette attaque requiert une machine plus puissante (surtout, avec une meilleure alimentation en énergie) qu'un capteur, mais permet à la fois de créer des congestions\index{congestion} dans le réseau, et de saturer les capacités des capteurs en ouvrant un nombre trop grand de sessions \tcp.
        ...
        \paragraph{Désynchronisation\index{desynchro@désynchronisation} \tcp (ou équivalents)}
...
%{{{2
La \idx{couche application} implémente éventuellement l'application utilisée au niveau le plus haut par le \rc pour assurer un service en particulier.
...
\paragraph{Désynchronisation\index{desynchro@désynchronisation}}
Si jamais le protocole utilisé sur la \idx{couche application} induit la création de sessions en mode connecté (comme pour \tcp sur la \idx{couche transport}), des attaques de \desync peuvent être menées par un attaquant en vue de briser ces sessions~\cite{AD14}.
...
C'est l'une des manières de s'emparer d'un capteur pour le reprogrammer et introduire un agent corrompu dans le réseau (l'autre méthode reposant sur des failles logicielles, qui permettent parfois l'\idx{injection} distante de code).
Mais sans s'embarrasser de les reprogrammer ou d'en épuiser les ressources, l'attaquant qui obtient un accès physique au réseau peut aussi se contenter de détruire physiquement\index{destruction physique} les capteurs, ce qui est encore le moyen le plus efficace de les empêcher de fournir les services pour lesquels ils ont été conçus.
...
\begin{table}[!ht]
    ...
    \begin{small}
        \tabulinesep=.3ex%
        \begin{tabu}to \linewidth {X[2,c,m]|X[7,l,m]}
            \toprule
            ...
            \midrule
            \multirow{4}{*}{\parbox{.16\textwidth}{\centering Couche liaison de données}}%
...
\end{table}
\begin{table}[!ht]
    \caption[Classement des attaques par paradigme]{Classement des attaques par paradigme (voir \sssref{ea:sss:paradigm})}\label{ea:tab:paradigm}
    \centering
    \medskip
    \begin{small}
        \begin{tabu}{X[1,l,m]|X[2,j,m]}
            ...
            \bottomrule
         \end{tabu}
         \index{destruction physique}\index{agrégation}\index{altération}\index{brouillage}\index{collision}\index{brouillage!brouillage intelligent}\index{comportement cupide!accaparement}\index{falsification}\index{trou!trou noir}\index{trou!trou gris}\index{trou!trou on/off}\index{routage}\index{routage!boucle}\index{trou!puits}\index{trou!trou de ver}\index{attaque Sybil}\index{deluge@déluge de paquets}\index{desynchro@désynchronisation}\index{congestion}\index{usurpation d'identité}
     \end{small}
\end{table}

Il est bien sûr très difficile de prétendre à l'exhaustivité, et il existe d'autres attaques par \dds applicables aux \rcs: certaines dépendent d'applications particulières ou sont introduites par des configurations précises (\idx{mobilité} des nœuds, structures spécifiques: grille, recherche de couverture maximale\dots).
D'autres n'ont sans doute pas encore été découvertes.
Les attaques que nous avons présentées regroupent néanmoins les principales menaces, en matière de \dds, auxquelles peuvent avoir à faire face les \rcs.
%2}}}
...
\begin{itemize}
    ...
    \item \textbf{Altérer\index{altération} les résultats}: l'attaquant souhaite induire sa cible en erreur en lui faisant parvenir des résultats erronés. Les prises de décision de l'opérateur vont alors se retrouver basées sur une vision erronée de la situation en cours. C'est l'exemple classique des séries d'espionnage, dans lesquelles les agents infiltrés font en sorte d'afficher l'image d'une pièce vide dans la salle de contrôle des dispositifs de vidéosurveillance afin de camoufler leur intrusion.
\end{itemize}
...
Aussi le \idx{brouillage}, attaque externe, semble préférable, car plus simple, à bon nombre d'attaques plus complexes à mettre en place et nécessitant un accès dans le réseau.
Il semble donc plus rentable que les attaques portant sur les protocoles de \idx{routage}, ou bien les attaques de couche supérieure (par \desync ou création de congestions\index{congestion}), elles aussi destinées à rendre le réseau hors service.
...
La \idx{destruction physique} nécessite de connaitre l'emplacement\index{localisation géographique!emplacement} des capteurs.
...
En revanche, la collecte parasite elle-même peut passer par des attaques sur le protocole de \idx{routage}, de façon à détourner le plus de données utiles vers le nœud corrompu.
L'annonce de fausses routes\index{routage!route}, typiques des attaques \textit{sinkhole}\index{trou!puits}, se prêtent particulièrement bien à la collecte frauduleuse de paquets (d'ailleurs, une fois ces paquets collectés par l'attaquant, à quoi bon les retransmettre?).

En comparaison, les attaques de type \textit{blackhole}\index{trou!trou noir} ou \textit{grey hole}\index{trou!trou gris} se limitent à la suppression de paquets, sans influer sur la création des routes\index{routage!route}.
Elles peuvent être utilisées pour neutraliser la remontée d'informations, mais une fois encore, on leur préfèrera dans ce contexte des attaques de \idx{brouillage}.

            \subparagraph{Altérer\index{altération} les mesures}
Les attaques portant sur l'\integrite des données sur les différentes couches semblent efficaces pour altérer\index{altération} les résultats perçus par l'exploitant.
Il y a toutefois une limite relative à l'application pour laquelle le réseau a été déployé: si les résultats finaux sont similaires à des moyennes calculées sur toute la zone couverte par les capteurs, influer significativement sur cette moyenne va demander d'altérer\index{altération} un grand nombre de paquets.
...
La modification\index{altération} des paquets peut être menée sur un grand nombre d'entre eux pour peu que le nœud compromis soit bien situé, \cad s'il s'agir d'un relai proche de la \sdb.
