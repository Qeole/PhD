...
\end{itemize}
Les mécanismes généralement impliqués tiennent bien sûr du domaine de la \idx{cryptographie}, et doivent être intégrés aux protocoles de sécurité utilisés~\cite{DSK10}.
...
\begin{table}[ht] %{{{
    ...
    \begin{footnotesize}
        ...
        \end{tabu}
    \end{footnotesize}
    \index{SPINS}\index{TinySec}\index{TinyKey}\index{MiniSec}\index{LEAP}\index{LEAP+}\index{pDCS}\index{ERP-DCS}\index{architecture de sécurité}\index{chiffrement}\index{AES}\index{intégrité}\index{authentification}\index{rejeu}
\end{table}
%}}}
