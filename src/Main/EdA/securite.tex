...
\begin{itemize}
    ...
    \item la \idx{disponibilité}, c'est à dire la résistance aux attaques de \dds. Cette problématique est centrale pour les travaux présentés ici, et sera développée plus largement que les autres dans la \sref{ea:sec:dos};
\end{itemize}
...
Idéalement, pour des \rcs, ces algorithmes doivent être implémentés de telle façon que soit respectées deux sous-problématiques liéés au \idx{chiffrement}:
\begin{itemize}
...
\end{itemize}
Nous n'entrerons pas plus en détail dans les mécanismes cryptographiques\index{cryptographie} impliqués.
...
\subsection{Intégrité, \idx{authentification}, \idx{non-répudiation}, \idx{rejeu}}
...
L'\idx{authentification} regroupe en réalité à la fois l'\idx{authentification} de deux parties l'une envers l'autre, et l'\idx{authentification} d'un agent aux yeux de tous les autres membres du réseau, dans le cas d'une diffusion générale (\textit{broadcast}) des messages.
...
\nomenclature{MSPQS}{\textit{Modified Secured Query Processing Scheme}}
...
On entend par « \idx{architecture de sécurité} » un ensemble de protocoles et de mécanismes déployés dans le réseau en vue d'apporter certaines garanties en termes de \secu.
...
Ainsi \idx{SPINS} (\textit{Security Protocols for Sensor Networks}), élaboré en 2002~\cite{PSWCT02}, est l'une des premières solutions proposées dans la littérature pour sécuriser les échanges dans les \rcs, et sert de base pour de nombreuses solutions plus récentes.
...
\idx{TinySec}~\cite{KSW04}, proposé en 2004, se réfère à \idx{SNEP}, mais lui reproche une spécification incomplète.
...
\idx{TinyKey}~\cite{CRS11} est une autre architecture basée sur \idx{TinySec}, mais date de 2011.
...
Avec \idx{LEAP} (\textit{Localized Encryption and Authentication Protocol}, « protocole de \idx{chiffrement} et d'\idx{authentification} localisés »), proposé en 2003~\cite{ZSJ03}, la gestion est décentralisée, et porte sur quatre types de clés:
\begin{itemize}
...
Ils ont été améliorés dans une seconde version du protocole, baptisée \idx{LEAP+}, et publiée en 2006~\cite{ZSJ06}.
\nomenclature{LEAP}{\textit{Localized Encryption and Authentication Protocol}}

L'architecture \idx{pDCS}, proposée en 2009~\cite{SZZCY09} (\textit{privacy-enhanced Data-Centric Sensor networks}, « \rcs centrés sur les données, de \idx{confidentialité} améliorée ») divise le réseau en cellules rectangulaires à l'aide d'arbres euclidiens de \textsc{Steiner}, et introduit des clés de cellules et de rangées.
...
On peut également observer dans la littérature que des mécanismes introduits dans les architectures complexes présentées ci-dessus ont été repris dans des solutions ultérieures.
