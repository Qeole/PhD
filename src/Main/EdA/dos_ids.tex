...
Lorsque le \ch reçoit plusieurs rapports de la part de \textit{gNodes}\index{gNodes@\textit{gNodes}} distincts (ceci afin de limiter les cas de faux positifs), le \CH en vient à considérer le capteur dénoncé comme étant corrompu et prend des mesures en conséquences.
...
C'est sur ces points que se concentrent la majeure partie des travaux présentés dans cette thèse: les \cns que nous utilisons ne sont qu'une version renommée des \textit{gNodes}\index{gNodes@\textit{gNodes}}, pour lesquels a été proposé dans un premier temps un \idx{renouvellement} périodique~\cite{GMT12}.
