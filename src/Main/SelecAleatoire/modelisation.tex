...
Nous venons d'établir un processus de sélection aléatoire, périodiquement renouvelé, des \cns assurant la détection des nœuds compromis dans le réseau.
Voici à présent plusieurs modèles possibles permettant de représenter cette solution, et d'en évaluer les performances selon divers critères.
...
\begin{figure}[H]
    ...
    \caption{Équation de transition du processus de \textsc{Markov} à temps continu modélisant le cluster}\label{sa:fig:eqtrans}
\end{figure}
...
Une autre représentation possible système passe ainsi par l'utilisation de processus stochastiques à évènements discrets (PSED, en anglais \textit{Discrete-Event Stochastic Processes}, ou encore \textit{Generalized Semi-\textsc{Markov} Processes} pour « processus semi-markoviens généralisés »).
...
Ces arcs sont rattachés à la place \textsf{InBuff$_{i_j}$}\,\footnote{Pour \textit{in buffer}.} de chacun des nœuds voisins du nœud $i$ (on trouve donc autant d'arcs sortants que de nœuds voisins).
...
Le \ch n'est considéré que comme un simple puits, uniquement équipé d'un tampon mémoire d'entrée pour recevoir les données.
\begin{figure}[ht]
    \centering
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item $\mbox{last}(y)$ est la dernière des valeurs endossées par l'expression $y$ le long d'un chemin synchronisé; $\mbox{min}(y)$ et $\mbox{max}(y)$ sont les valeurs optimales obtenues le long de ce chemin. $\mbox{int}(y)$ est l'intégrale d'$y$, et $\mbox{avg}(y)$ la moyenne des valeurs obtenues le long du chemin.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item la première transition qui boucle sur $e_1$: $e_1\xrightarrow{\mathit{true},\{\mathsf{chkYES_i}\},(x_{d_i}:=x_{d_i}+1)}{e_1}$.
        Elle correspond à l'évènement associé au passage de la transition \textsf{check\_YES} du nœud $i$ sur le modèle \rpsge du cluster (présenté en \figref{sa:fig:petridyn}).
