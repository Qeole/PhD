...
La figure~\ref{sa:fig:detec-nbcn} présente le taux de détection du nœud compromis pour différentes quantités de \cns dans le réseau, par période.
...
La courbe présentée en figure~\ref{sa:fig:detection-txRate} illustre cet état de fait.
\begin{figure}[H]
    \centering
    \includegraphics{\chapterfig/plot_detection-txRate.pdf}
    \caption{Taux de détection en fonction de $\lambda$}\label{sa:fig:detection-txRate}
\end{figure}
